"""Runs one agent episode against a prepared workspace, via the Claude Agent
SDK backend.

Must run under .agent-study-venv-runner (has claude-agent-sdk installed):

    .agent-study-venv-runner/bin/python -m agent_study.run_agent_claude_agent_sdk <workspace_dir> <condition>
...
async def _run_agent(workspace_dir: Path, condition: str, prompt: str, family: str = "prompt_ab") -> dict:
    ...
    # wrapper) without ever running real evalstats code. `attempted_evalstats_import`
    # (already used for the tool_summary's python-libs list via IMPORT_RE) is
    # a tighter signal: it only fires on an actual `import evalstats`/`from
    ...
    result_meta["used_evalstats"] = used_evalstats_tool or attempted_import
    result_meta["used_evalstats_tool_call"] = used_evalstats_tool
    result_meta["attempted_evalstats_import_in_bash"] = attempted_import
    result_meta["n_bash_commands"] = len(bash_commands)
    result_meta["n_containment_violations"] = len(containment_violations)
    result_meta["tool_summary"] = summarize_tool_use(transcript_events, bash_commands, result_meta["used_evalstats"])
...
if __name__ == "__main__":
    if len(sys.argv) not in (3, 4):
        print("usage: run_agent_claude_agent_sdk.py <workspace_dir> <condition: full|baseline> "

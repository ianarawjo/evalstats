...
Utilities for statistically sane, dead-simple analyses for comparing prompt and LLM performance. 
...
The idea is that you provide `prompt-stats` with your data, and we run more statistically appropriate tests quantifying uncertainty and providing confidence bounds on claims. 
...
`prompt-stats` aims to rectify this with simple, powerful defaults—just throw us your data and we'll run the stats and plot the results for you. Upstream applications, like LLM observability platforms, could take `prompt-stats` results and plot them in their own front-ends. Prompt optimization tools could also use `prompt-stats` to decide, e.g., when to cull a candidate prompt and how to present results to users. 
...
A common failure mode in LLM benchmarking — both in academic papers and practitioner evaluations — is testing each model with a single prompt template and reporting the resulting scores as if they reflect stable model capabilities. In reality, model rankings frequently flip under semantically equivalent paraphrases of the same instruction. A benchmark result that says "Model A beats Model B" may be an artifact of prompt phrasing, not a meaningful capability difference.
...
We aim to iteratively contribute to `prompt-stats`. Ideas for future features:
